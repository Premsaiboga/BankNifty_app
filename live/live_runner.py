    ...
    while True:
        ...
        if tick_minute > current_minute:
            ...
            if candle:
                ...
                print("🕯 New 1-min Candle:", candle["datetime"])
                ...
                if len(minute_candles) % 5 == 0:
                    ...
                    print("🔥 New 5-min Candle:", five["datetime"])
                    ...
                    # ===== VWAP FIXED RR =====
                    RR = 4
                    vwap_trades = vwap_strategy.generate_signals(df)
                    ...
                    for t in vwap_trades:
                        ...
                        if t["type"] == "BUY":
                            t["target"] = t["entry"] + (t["entry"] - t["stoploss"]) * RR
                        else:
                            t["target"] = t["entry"] - (t["stoploss"] - t["entry"]) * RR

                        t["features"] = {
                            "vwap_distance": abs(t["entry"] - df.iloc[-1]["vwap"]),
                            "candle_size": df.iloc[-1]["high"] - df.iloc[-1]["low"],
                            "atr": df.iloc[-1]["atr"],
                    ...
                    for trade in trades:
                        ...
                        print(f"🎯 Trade Found: {trade['strategy']} {trade['type']}")
                        process_trade(trade)
